...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($quotation_no)) {
        ...
        // Kiểm tra trùng số báo giá (bỏ qua chính nó)
    ...
    if (empty($error)) {
        ...
        try {
            ...
            $stmt = $conn->prepare(
                "UPDATE quotations SET quotation_no=?, customer_id=?, issue_date=?, valid_until=?, currency=?, exchange_rate=?, notes=?, status=?
                 WHERE id=?"
            );
                            ...
                            <tr class="item-row">
                                ...
                                <td>
                                    <input type="text" class="form-control form-control-sm item-amount bg-light"
                                           readonly value="<?php echo htmlspecialchars(number_format(floatval($item['amount'] ?? 0), 4, '.', '')); ?>">
                                </td>
                                <td>
...
function calcAmount(row) {
    ...
    row.querySelector('.item-amount').value = (price * qty).toFixed(4);
}
